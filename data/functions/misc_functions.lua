...
function pseudorandom_element(_t, seed)
  ...
  local randInd = math.random(#keys)
  local key = keys[randInd].k
  return _t[key], key 
end
...
-- å³ç§»ï¼ˆé€»è¾‘å³ç§»ï¼Œé«˜ä½è¡¥0ï¼‰
function bitrshift(x, n)
  return math.floor(x % 2^32 / 2^n)
end

-- å¼‚æˆ–ï¼ˆæŒ‰ä½å¼‚æˆ–ï¼‰
function bitbxor(a, b)
  local res = 0
  for i = 0, 31 do
    local bit_a = a % 2
    local bit_b = b % 2
...
function pseudohash(str)
  if true then 
    ...
    for i=#str, 1, -1 do
      h = bitbxor(h, bitlshift(h, 7) + bitrshift(h, 3) + string.byte(str, i))
    end
