...
function printTable(tbl, indent)
    ...
    for key, value in pairs(tbl) do
        if type(value) == "table" then
            print(indentStr .. key .. ":")
            printTable(value, indent + 1)  -- é€’å½’æ‰“å°åµŒå¥—è¡¨
        else
...
function Game:set_globals()
    ...
    -- local mode = love.window.getMode()
    -- -- æå–çª—å£çš„å®½åº¦å’Œé«˜åº¦
    -- local width = mode.width
    -- local height = mode.height
    local width, height = love.graphics.getPixelDimensions()  -- æˆ–è€…ä½¿ç”¨ love.window.getMode()
    print("width.."..width..".."..height)
    -- åˆ¤æ–­è®¾å¤‡ç±»åž‹
    if width < 1024 then
        print("Mobile device detected!")
    else
        print("PC device detected!")
    end
    ...
    if love.system.getOS() == 'iOS' or love.system.getOS() == 'Android' then
        ...
        if love.system.getPlatform and love.system.getPlatform() == "tvos" then
            ...
            self.F_RUMBLE = (love.system.getOS() ~= 'Android' and 1 or nil)
        end
        
        self.F_BABYNAMES = (love.system.getOS() == 'Android' and true or nil)
        self.F_CRASH_REPORTS = false
        self.F_SOUND_THREAD = false
        self.F_QUIT_BUTTON = false
        self.F_MOBILE = true
    ...
    if love.system.getPlatform and love.system.getPlatform() == 'tvos' then 
        self.F_CONTROLLER_REQUIRED = true
    ...
    self.SETTINGS = {
        ...
        WINDOW = {
            screenmode = "Windowed", -- 'Borderless',
            vsync = 1,
            selected_display = 1,
            display_names = {'[NONE]'},
            DISPLAYS = {
        ...
        GRAPHICS = {
            ...
            crt = 0, -- æœ€å‰ç«¯æ¡çº¹æ•ˆæžœåŽ»æŽ‰
            bloom = 1
        },
    }
    printTable(love.window)
    if width < 1024 then
        self.SETTINGS.WINDOW.screenmode = "Borderless"
        -- love.window.setMode(width, height, {minwidth = width, minheight = height, highdpi = true})
    else
        width = 1100
        height = 620
        love.window.setMode(width, height, {minwidth = width, minheight = height})
    end
